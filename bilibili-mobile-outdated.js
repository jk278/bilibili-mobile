...
(function () {
  ...
  function getIdByTitle(keyword, onResult) {
    ...
    window[callbackName] = function (responseData) {
      const bvId = responseData?.data?.result[11]?.data[0];
      console.log('test data: ', bvId);
      if (bvId) {
        onResult(bvId);
      } else {
  ...
  function addTargetElementListener(targetElement) {
    if (targetElement) {
      ...
      const keyword = encodeURIComponent(h2Element.innerHTML); // 已编码一次
  ...
  function updateMenuText(isAutoPlay) {
    const menuText = isAutoPlay ? "禁用自动播放" : "启用自动播放";
    return menuText;
  }

  function toggleFeature() {
    // 获取当前选项的状态
    const isAutoPlay = GM_getValue('isAutoPlay', true);
    ...
    console.log('test isAutoPlay: ', isAutoPlay);
    // 更新菜单项文本以显示当前状态
    const updatedMenuText = updateMenuText(!isAutoPlay);
    console.log('test updatedMenuText: ', updatedMenuText);
  ...
  function getAutoPlayValue() {
    if (typeof GM_getValue === 'function') {
      console.log('test getAutoPlayValue: ', GM_getValue('isAutoPlay', true));
      return GM_getValue('isAutoPlay', true);
    } else {
      console.log('test getAutoPlayValue: true');
      // 在不支持 GM_getValue 的环境中使用默认值（true）
      return true;
    }
  }
  ...
  executeAfterDOMContentLoaded(() => {
    ...
    // 注册初始菜单项
    const initialIsAutoPlay = GM_getValue("isAutoPlay", true);
    const initialMenuText = updateMenuText(initialIsAutoPlay);
    GM_registerMenuCommand(initialMenuText, toggleFeature, "i");
