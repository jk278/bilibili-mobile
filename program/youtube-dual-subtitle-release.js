...
(function () {
  ...
  const localeLang = navigator.language.split('-')[0] || 'en' // 跟随 YouTube 页面所用语言
    ...
    ah.proxy({
      ...
      onResponse: (response, handler) => {
        ...
        if (response.config.url.includes('/api/timedtext') && !response.config.url.includes('&translate_h00ked')) {
          const xhr = new XMLHttpRequest() // 创建新的 XMLHttpRequest
          ...
          if (isOfficialSub) {
            ...
            const pureLocalEvents = localeJson.events.filter(event => event.aAppend !== 1 && event.segs)
            for (const defaultJsonEvent of defaultJson.events) {
              if (!defaultJsonEvent.segs) continue
              const currentStart = defaultJsonEvent.tStartMs
              const currentEnd = currentStart + defaultJsonEvent.dDurationMs
              const currentLocalEvents = pureLocalEvents.filter(pe => currentStart <= pe.tStartMs && pe.tStartMs < currentEnd)
              let localLine = ''
